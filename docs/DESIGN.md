...
## Этап 16-17 (объединено): Восстановление после перезагрузки ✅
**Цель**: При перезагрузке Home Assistant восстанавливать режим согласно физическому переключателю

**Подход**: Без сохранения состояния - система всегда стартует согласно положению физического переключателя

### 16.1 Обработка неопределённого состояния при загрузке
- [x] В `_determine_switch_mode()` изменить ERROR на WARNING для "unknown" состояний
- [x] Это нормально во время запуска HA - датчики ещё не инициализированы
- [x] Система временно использует безопасный режим "low"
- [x] Автоматически обновляется на корректный режим когда переключатель станет доступен

### 16.2 Что НЕ делаем
- [x] НЕ используем RestoreEntity
- [x] НЕ сохраняем состояние между перезагрузками
- [x] НЕ восстанавливаем auto-boost, manual-boost
- [x] НЕ сохраняем статистику (счётчики boost сбрасываются)

### 16.3 Поведение при загрузке
- [x] HA запускается → переключатель в состоянии "unknown"
- [x] Система временно устанавливает режим "low" (безопасный режим)
- [x] Переключатель становится доступен → система читает реальное положение
- [x] Режим автоматически обновляется согласно физическому переключателю
- [x] Все boost-состояния сбрасываются (начинается с чистого листа)
...
### 23.1 README.md
- [x] Описание компонента
- [x] Требования (Home Assistant версия, устройства)
- [x] Инструкция по установке через HACS
- [x] Пример конфигурации в configuration.yaml
- [x] Описание режимов работы
- [x] Описание сервисов

### 23.2 Примеры автоматизаций
- [x] Пример автоматизации с force_boost по кнопке
- [x] Пример автоматизации по расписанию
- [x] Пример использования в Lovelace UI

### 23.3 Troubleshooting
- [x] Как проверить логи
- [x] Частые проблемы и решения
- [x] Как проверить состояние датчиков
...
Обработка перезагрузки и логирование
